...
> **Repository location:** This file is the **canonical copy in the repo root** so it is easy to find (Cursor, Copilot, humans). Keep it here; do not maintain a competing full copy under `markdown/`.
...
├── dashboardStore.ts   # Custom dashboard layouts (global/project), widget lanes
...
| `store/dashboardStore.ts` | Custom dashboards, Overview is separate (see below) |
...
- **`AGENTS.md`** (this file, **repo root**) — primary agent & contributor guide
...
- `markdown/AGENTS.md` - Redirect stub; full content is **`AGENTS.md` at repo root**
